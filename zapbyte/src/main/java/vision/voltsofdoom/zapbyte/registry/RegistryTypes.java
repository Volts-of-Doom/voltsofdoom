...
public class RegistryTypes {
  private static volatile LinkedHashMap<ResourceLocation, RegistryType<?>> types =
      new LinkedHashMap<ResourceLocation, RegistryType<?>>();
  ...
  /**
   * Creates and registers a new {@link RegistryType}, which is necessary for adding objects to the
   * game which do not exist in the {@link VoltsOfDoomCoreSystem}.
   * 
   * @param identifier The {@link ResourceLocation} identifier for the created {@link RegistryType}.
   * @param clazzType The {@link Class} of object which will be registered by {@link IRegistry}s of
   *        the created {@link RegistryType}.
   * @return The completed {@link RegistryType}
   */
  public static synchronized <T extends IRegistryEntry<T>> RegistryType<T> create(
      ResourceLocation identifier, Class<T> clazzType) {

    Objects.requireNonNull(identifier,
        () -> "Cannot create a RegistryType with a null identifier.");
   ...
   * 
   * @param type
   * @return The {@link ResourceLocation} of the given {@link RegistryType}, if it exists.
   */
